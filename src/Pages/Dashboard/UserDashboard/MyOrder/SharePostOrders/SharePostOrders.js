...
import '../MyOrder.css'
import useAuth from '../../../../../hooks/useAuth';
import ShareCustomer from './ShareCustomer';
import CartOrder from '../MyBooking';


const SharePostOrders = () => {
    ...
    useEffect(()=>{
        fetch(`https://burger-backend-production.up.railway.app/myOrder/${user?.email}`)
        .then(res=>res.json())
        .then(data=>{
            setOrder(data)
        })
    ...
    return (
        <Container>
            <Toolbar />
            <Divider className='' style={{color:"red"}}>
                <Fab className='' variant="extended" size="small" color={color} aria-label="add">
...
export default SharePostOrders;